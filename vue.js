...
new Vue({
    ...
    computed: {
        filteredList() {
          ...
          }else if(this.searching.length>=3){
            return this.postList.filter(post => {
              return post.title.toLowerCase().includes(this.searching.toLowerCase())
          })
          }
    ...
    watch: {
        searching: function() {
            if(this.searching.length>=3 && this.filteredList.length != 0){
              this.status = `Looking for : ${this.searching}`
            }else if(this.searching.length == 0){
              this.status = ''
